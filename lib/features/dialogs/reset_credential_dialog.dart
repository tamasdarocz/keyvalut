  ...
  Future<void> _selectAndLoadRecoveryKey() async {
    try {
      ...
      if (result != null && result.files.single.path != null) {
        ...
        if (kDebugMode) {
          print('Read encrypted core from file: $encryptedData');
        }
        ...
        if (decryptedContent['type'] != 'recovery_key' || decryptedContent['core'] == null) {
          throw Exception('Invalid recovery key file: Missing or incorrect core');
        }

        // Update the UI after async work is complete
        setState(() {
          _recoveryKeyController.text = decryptedContent['core'];
        });
